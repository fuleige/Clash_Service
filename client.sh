...
After install, new terminals can use:
  clash_service start
  clash_service stop
...
EOF
  ...
  log "已在 ${rc_file} 加入受控代理 loader 和 clash_service 函数。"
}
...
write_proxy_enabled_env() {
  ...
  local_proxy_port="$(current_local_proxy_port)"
  http_proxy_url="http://127.0.0.1:${local_proxy_port}"
  all_proxy_url="socks5://127.0.0.1:${local_proxy_port}"
  mkdir -p "$STATE_DIR"
...
EOF
  ...
  if [ "${CLASH_SERVICE_SHELL_FUNCTION:-0}" = "1" ]; then
    log "clash_service 命令结束后会同步当前终端代理变量。"
  else
...
main() {
  ...
  case "$command" in
    ...
    start) start_client ;;
    stop) stop_client ;;
    restart) require_non_root; restart_service; check_proxy_connectivity; write_proxy_enabled_env ;;
